...
class BaseModelGenerator(object):
    ...
    def __init__(self, params):
        ...
        self.def_string = '    %(c_name)s = Column(%(type)s, nullable=%(nullable)s,unique=%(uniq)s,default=%(default)s)'
        self.def_string_pk1 = '    %(c_name)s = Column(%(type)s, nullable=%(nullable)s,unique=%(uniq)s,default=%(default)s, primary_key=%(primary_key)s)'
        self.def_string_pk2 = '    %(c_name)s = Column(%(type)s, nullable=%(nullable)s,unique=%(uniq)s,default=%(default)s, primary_key=%(primary_key)s, autoincrement=True)'
    ...
    def generate_class_string(self, table_name):
        ...
        print "class %s(Base):" % self.get_class_name(table_name)
        ...
        for i in auto_table.columns:
            ...
            elif isinstance(coltype, MSFloat):
                coltype = "types.Float()"
            default = None
            if hasattr(i, "server_default"):
                if hasattr(i.server_default, "arg"):
                    default = i.server_default.arg
                    if isinstance(i.type, (MSString, MSText, MSDateTime, MSTimeStamp, MSDate)):
                        default = repr(unicode(default))
...
#        print "    pass"
    ...
    def generate_class_init_string(self, table_name):
        auto_table = self.auto_fetch_table(table_name)
        self.check_for_pk(auto_table)
        columns = []
        ...
        for i in auto_table.columns:
            ...
            if hasattr(i, "server_default"):
                if hasattr(i.server_default, "arg"):
                    ...
                    if isinstance(i.type, (MSString, MSText, MSDateTime, MSTimeStamp, MSDate)):
                        default = repr(unicode(default))
            columns.append((i.name, default)) 
        
        print "    def __init__(self, %s):" % ", ".join(['%s=%s' % itm for itm in columns])
        for itm in columns:
            print "        self.%s = %s" %(itm[0], itm[0])
    ...
    def generate_model(self):
        ...
        for table_name in self.params.table_list:
            ...
            print ""
            self.generate_class_init_string(table_name)
            print ""
            print ""
        print ""
